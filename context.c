...
void upf_sess_urr_acc_add(upf_sess_t *sess, ogs_pfcp_urr_t *urr, size_t size, bool is_uplink)
{
    ...
    ogs_info("************ siztel upf_sess_urr_acc_add ************");
...
ogs_info("Session ID: %od, APN: %s, UE IP: %s, Total Octets: %ld, UL Octets: %ld, DL Octets: %ld, Total Packets: %ld, UL Packets: %ld, DL Packets: %ld",
         urr->id, sess->apn_dnn, 
         sess->ipv4 ? OGS_INET_NTOP(&sess->ipv4->addr, buf1) : (sess->ipv6 ? OGS_INET6_NTOP(&sess->ipv6->addr, buf2) : "N/A"),
         urr_acc->total_octets, urr_acc->ul_octets, urr_acc->dl_octets,
         urr_acc->total_pkts, urr_acc->ul_pkts, urr_acc->dl_pkts);
